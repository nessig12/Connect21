...
from app import db
from app.main.forms import EditProfileForm, PostForm, AddTopic
from app.models import User, Post, Topics
...
def pages():
    ...
    next_url = url_for('main.pages', page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('main.pages', page=posts.prev_num) \
...
def explore():
    ...
    if request.method == 'POST':
        # print(request)
        topic = Topics(topic=form1.topic.data)
        db.session.add(topic)
        db.session.commit()
        flash('Topic added:  {}'.format(
            form1.topic.data))
    ...
    return render_template('explore.html', title=_('Explore'),
                           posts=posts.items, topics= topics, next_url=next_url,
...
def topic_page():
    ...
    prev_url = url_for('main.index', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('topics.html', title=_('Topics'), form=form,
                           posts=posts.items, next_url=next_url,
                           prev_url=prev_url)
